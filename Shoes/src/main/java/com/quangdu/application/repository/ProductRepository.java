package com.quangdu.application.repository;
...
//    @Query(value = "SELECT NEW dto.model.com.quangdu.application.ProductInfoDTO(p.id, p.name, p.slug, p.price ,p.images ->> '$[0]', p.total_sold) " +
//            "FROM product p " +
//            "WHERE p.status = 1 " +
